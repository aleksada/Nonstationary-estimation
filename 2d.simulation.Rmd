...
##Function for simulating nonstaionary covariance grided on [0,1]^2 given spatially varying parameters
```{r simulate cov}
##Input:
...
##Simulate the GRF and show some nonstationary features
```{r 2d initial}
library(convoSPAT)
library(geoR)
library(fields)
...
##Estimating beta0 using convoSPAT::NSconvo_fit function
```{r 2d step 1}
...
##Estimating beta1 and beta2 based on different partitions.
```{r 2d step 2 option1, eval = FALSE}
## Divide by Square
nk <- N.mc
Nk <- sqrt(nk)
nkr <- n/nk
...
##Smooth the parameters and show results
```{r 2d results}
...
##Function for repeating estimations 100 times in parallel
```{r func repeat}
library(parallel)
...
##Show some comparisons based on 100 estimators
...
#ns.result<-readRDS("/Users/liy0h/Dropbox/My Research/Nonstationary paper/code/Final code/2dsims-100-new.rds")
...
title(ylab = list("NS1 estimates", cex = 2, font = 3),
      xlab = list(expression(s[1]), cex = 1, font = 3))
contour(exp(lam1.est.ll),add = T)
